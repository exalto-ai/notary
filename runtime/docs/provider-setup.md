...
Exalto Capture supervises `notaryd`, which exposes provider-compatible
HTTP/1.1 routes on one loopback listener. Codex CLI and Claude Code can keep
their saved product sign-ins. API and SDK clients can send their existing
provider key or use the scoped local token created after a validated provider
key is imported into macOS Keychain. In every case, replace only the base URL.
...
page. The app offers two per-client authentication paths:

- **Use scoped local token** validates a provider key over HTTPS, saves it in
  macOS Keychain, and creates a separate local token for the client. The
  supervised service loads the provider key into memory and substitutes it
  only for the exact token on the matching route. A reserved local token in
  any other header is rejected locally.
- **Send provider key** leaves the key in the SDK, CLI, shell, or secret
  manager. Exalto Capture does not store a separate copy of the
  request-supplied key.

The paths can coexist. Selecting **Send provider key** does not disable a key
imported earlier. Remove that saved key in setup to disable its scoped-token
path.
