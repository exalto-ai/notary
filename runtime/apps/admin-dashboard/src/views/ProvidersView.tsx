  ...
  return (
    <Paper className="settings-panel provider-route">
      ...
      <Text className="safe-note">
        <ShieldCheck size={15} />
        {codex
          ? ' Codex CLI keeps the saved ChatGPT login and sends it with requests. No API-key import is needed for this route.'
          : ' Claude Code keeps the saved claude.ai login and sends it with requests. No API-key import is needed for this route.'}
      </Text>
      <details className="notary-details">
        ...
        <pre className="json-view">{codex ? CODEX_CONFIG : CLAUDE_COMMAND}</pre>
        <Button
          variant="outline"
          leftSection={<Copy size={14} />}
          onClick={() =>
            onCopySetup(
              codex ? 'Codex CLI config' : 'Claude Code command',
              codex ? CODEX_CONFIG : CLAUDE_COMMAND,
            )
          }
        >
...
function ApiRouteCard({
  ...
  const supportsKeychainImport = ['openai', 'anthropic', 'openrouter'].includes(route.id);
  return (
    <Paper className="settings-panel provider-route">
      <Group justify="space-between" align="flex-start">
        <div>
      ...
      <Text className="safe-note">
        ...
        {supportsKeychainImport
          ? ' You can instead import this key through the Exalto Capture setup assistant. Imported keys are validated directly with the provider and stored in macOS Keychain; the client receives a scoped local token.'
          : ' Keychain import is not available for this route.'}{' '}
...
export function ProvidersView({
  ...
  const copyBaseUrl = async (label: string, baseUrl: string) => {
    ...
    notifications.show({
      ...
      message:
        'Model selection stays in the tool. Keep its API key there, or use the scoped local token provided by Exalto Capture setup.',
    });
  };
      ...
      {!embedded && (
        <header className="view-heading">
          ...
          <Text>
            ...
            stays in that tool. Saved product logins stay there, while API keys can stay there or be
        ...
        <>
          ...
          <section className="settings-group" aria-labelledby="api-routes-title">
            ...
            <Text className="provider-boundary-note">
              ...
              URL, then keep its API key in that client or import a supported key through the Exalto
              Capture setup assistant. A Keychain import gives the client a scoped local token, not
              the provider key. Model configuration stays in the client.
            </Text>
