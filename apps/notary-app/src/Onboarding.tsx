...
const clientChoices = [
  ...
  },
  {
    ...
    name: 'Codex',
    detail:
      'Configure a separate Codex session. It keeps your existing ChatGPT sign-in or API key.',
  },
  {
    id: 'claude',
    name: 'Claude Code',
...
export function createDisposableTestMarker() {
  ...
  return `${TEST_MARKER_PREFIX}${Array.from(bytes, (byte) => byte.toString(16).padStart(2, '0'))
    .join('')
    .toUpperCase()}`;
}
...
function expectedTestProvider(client: ClientId) {
  return client === 'claude' ? 'anthropic' : 'openai';
}

function testCommand(client: ClientId, prompt: string) {
  if (client === 'codex')
    return `codex --profile exalto-capture exec --ephemeral --skip-git-repo-check '${prompt}'`;
...
export function Onboarding({
    ...
    try {
      if (!state.vault_configured) {
        await configureVault(
          protectionMode,
          protectionMode === 'passphrase' ? passphrase : undefined,
        );
        setPassphrase('');
        setPassphraseConfirmation('');
        await refresh();
  ...
  const checkForTestTrace = async () => {
    ...
    try {
      ...
      const traceId =
        testBaseline === null
          ? null
          : await confirmDisposableTrace([...testBaseline], expectedProvider, testMarker, leaseId);
      if (!testWorkIsCurrent(operation, generation) || temporaryCaptureLease.current !== leaseId)
        return;
      if (traceId) {
        await restoreTestCapture(leaseId);
        if (!testWorkIsCurrent(operation, generation)) return;
  ...
  return (
    <div className="onboarding-window exalto-onboarding">
      <aside className="setup-rail">
        ...
        <ol
          className="setup-ledger"
          aria-label={`Setup step ${stepIndex + 1} of ${onboardingSteps.length}`}
        >
          {onboardingSteps.map((item, index) => {
            const content = (
              <>
                <span className="ledger-mark">
                  {index < stepIndex ? <Check size={11} /> : index + 1}
                </span>
                <span className="ledger-name">{stepNames[item]}</span>
                {index <= stepIndex && decisions[item] && (
                  <span className="ledger-value">{decisions[item]}</span>
                )}
              </>
            );
            return (
      ...
      <section className={`onboarding-content${step === 'client' ? ' is-client-step' : ''}`}>
        ...
        {step === 'welcome' && (
          <WelcomeStep state={state} onContinue={() => setStep('protection')} />
        )}
        {step === 'protection' && (
          <ProtectionStep
        ...
        {step === 'notary' && (
          <NotaryStep service={sealingService} onContinue={() => setStep('client')} />
        )}
        {step === 'client' && (
          <ClientStep
            client={client}
        ...
        {error && (
          <div
            className={`onboarding-error${step === 'client' ? ' client-step-error' : ''}`}
            role="alert"
          >
            {error}
          </div>
      ...
      <div className="wizard-step welcome-step">
        ...
        <p>
          {fresh
            ...
            : 'This Mac already has capture settings. Setup will preserve them while it checks the path from your AI tool to a portable trace.'}
        </p>
        <figure className="trace-receipt" aria-label="A sample local trace receipt">
          <figcaption>
            ...
            <code>TRACE / LOCAL</code>
          </figcaption>
          <dl>
            <div>
              <dt>Source</dt>
              <dd>Built-in chat or your AI tool</dd>
            </div>
            <div>
              <dt>Provider</dt>
              <dd>Authenticated response</dd>
            </div>
            <div>
              <dt>Private content</dt>
              <dd>Hidden from the sealing service</dd>
            </div>
            <div>
              <dt>Portable result</dt>
              <dd>.llmtrace</dd>
            </div>
          </dl>
          <p>
            A trace proves the interaction it contains. It does not prove that omitted interactions
            never happened.
          </p>
        </figure>
      </div>
  ...
  return (
    <>
      <div className="wizard-step">
        ...
        <p>
          A full private capture can reconstruct the original provider request, including
          credentials. Exalto Capture vault-encrypts that artifact before writing it to disk.
        </p>
        {configured ? (
          <div className="configured-protection">
            <BadgeCheck size={18} />
            <div>
              <strong>Local protection is already configured</strong>
              <span>Your existing vault will remain unchanged.</span>
            </div>
          </div>
        ) : (
          <div
            className="protection-options"
            role="radiogroup"
            aria-label="Private trace protection"
          >
            <button
              type="button"
              role="radio"
              aria-checked={mode === 'keychain'}
              className={mode === 'keychain' ? 'is-selected' : ''}
              onClick={chooseKeychain}
            >
              <span className="radio-mark">{mode === 'keychain' && <span />}</span>
              <div>
                <strong>Use macOS Keychain</strong>
                <p>
                  Recommended. macOS protects the vault key, with no separate password to remember.
                </p>
              </div>
            </button>
            {advancedOpen && (
              <button
                ...
                className={mode === 'passphrase' ? 'is-selected' : ''}
                onClick={() => setMode('passphrase')}
              >
                <span className="radio-mark">{mode === 'passphrase' && <span />}</span>
                <div>
                  <strong>Use a passphrase</strong>
                  <p>Enter it whenever the app opens. Exalto Capture does not save it.</p>
                </div>
              </button>
            )}
          </div>
        ...
        {!configured && (
          ...
          >
            Advanced protection <ChevronDown size={12} />
          </button>
        )}
        {!configured && advancedOpen && mode === 'passphrase' && (
          <div className="passphrase-fields">
            <label>
              ...
              <input
                ...
                value={passphrase}
                aria-invalid={!passphraseValid}
                aria-describedby={!passphraseValid ? mismatchId : undefined}
            ...
            {!passphrasePresent ? (
              <small id={mismatchId} className="passphrase-mismatch" role="alert">
                Enter a non-empty passphrase.
              </small>
            ) : (
              !passphrasesMatch && (
                <small id={mismatchId} className="passphrase-mismatch" role="alert">
        ...
        <div className="wizard-note" role="note">
          <LockKeyhole size={13} />
          <span>
            When retained previews are enabled, short prompt and response excerpts are also kept in
            local metadata so Traces can be browsed. Those excerpts stay on this Mac but are not
            protected by the trace vault.
          </span>
        </div>
      </div>
      <div className="wizard-actions">
    ...
    <>
      <div className="wizard-step notary-step">
        ...
        <div className="notary-choice is-selected">
          <span className="notary-choice-mark">
            <Check size={13} />
          </span>
          <div>
          ...
          <span className="choice-status">
            {service.isExaltoSeal && !service.configured
              ? 'Recommended'
              : service.available
                ? 'Configured'
                : 'Unavailable'}
        ...
        <dl className="notary-boundary">
          <div>
            <dt>Sealing service sees</dt>
            <dd>Provider hostname, encrypted traffic, sizes, timing</dd>
          </div>
          <div>
            <dt>Application plaintext</dt>
            <dd>Visible to this Mac and your chosen model provider</dd>
          </div>
        </dl>
        <button
          type="button"
        ...
        >
          About compatible notaries <ChevronDown size={12} />
        </button>
        {advancedOpen && (
          <div className="advanced-notaries">
            <div>
              <Server size={15} />
              <span>
                <strong>Compatible notary</strong>
                <small>Selected through signed Registry trust</small>
              </span>
              <em>Administrator managed</em>
            </div>
            <div>
              <SquareTerminal size={15} />
              <span>
                <strong>Self-hosted notary</strong>
                <small>Operator endpoint and verification key required</small>
              </span>
              <em>Administrator managed</em>
            </div>
            <p>
              This build preserves the pinned notary selected by its runtime configuration.
              Switching or adding a compatible notary requires an administrator-managed
              configuration.
            </p>
          </div>
        )}
      ...
      <div className="wizard-actions">
        <button className="mac-button is-primary is-large" type="button" onClick={onContinue}>
          {continueLabel}
        </button>
      </div>
    </>
  ...
  return (
    <>
      <div className="wizard-step client-step-scroll">
        ...
        <p>
          Chat inside Capture, or configure an external tool to send through it. You can add more
          connections later.
        </p>
        <div className="client-picker" role="radiogroup" aria-label="AI tool to connect first">
          {clientChoices.map((item) => (
            <button
              key={item.id}
              type="button"
              role="radio"
              aria-checked={client === item.id}
              className={client === item.id ? 'is-selected' : ''}
        ...
        <p className="harness-description">
          {clientChoices.find((item) => item.id === client)?.detail}
        </p>
        {client === 'builtin' ? (
          <ProviderConnections disabled={busy} />
        ) : (
          <AgentSetup
            key={client}
            client={client}
            prompt={agentSetupPrompt(client)}
            disabled={busy}
            manual={
              client === 'codex' ? (
                <>
                  <p>
                    Run <code>codex login status</code>. Keep its existing sign-in. For a ChatGPT
                    login, use the configuration below. For API-key authentication, use{' '}
                    <code>http://127.0.0.1:8787/openai/v1</code> as the base URL and{' '}
                    <code>env_key = "OPENAI_API_KEY"</code> instead of{' '}
                    <code>requires_openai_auth</code>. Set the key privately in your shell.
                  </p>
                  <pre>
                    <code>{CODEX_CONFIG}</code>
                  </pre>
                  <p>
                    Merge this into your user config, respecting <code>CODEX_HOME</code>. Start with{' '}
                    <code>codex --profile exalto-capture</code>. Desktop chats are not automatically
                    captured by this CLI profile.
                  </p>
                </>
              ) : (
                <>
                  <p>
                    Set <code>ANTHROPIC_API_KEY</code> privately in your shell or secret manager,
                    then launch a separate session using API billing.
                  </p>
                  <pre>
                    <code>{CLAUDE_COMMAND}</code>
                  </pre>
                  <p>
                    Claude Desktop can configure the CLI. Its own conversations do not use this
                    capture route.
                  </p>
                </>
              )
        ...
        {client !== 'builtin' && (
          <div className="wizard-note credential-capture-note" role="note">
            <LockKeyhole size={13} />
            <span>
              {externallyManagedService
                ? 'This separately managed service must already have capture on to run the optional test.'
                : 'Credentials stay in your tool. Private encrypted captures can retain credential-bearing request bytes; keep them secret.'}
            </span>
          </div>
        )}
      </div>
      <div className="wizard-actions client-step-actions">
        <button
          ...
          type="button"
          disabled={busy}
          onClick={client === 'builtin' ? onChat : onContinue}
        >
          {busy
            ? 'Preparing…'
            : client === 'builtin'
              ? 'Try a chat'
              : running
                ? 'Prepare optional test'
                : 'Start service and prepare test'}
        </button>
        <button className="mac-button is-large" type="button" disabled={busy} onClick={onSkip}>
          Continue without a test
        </button>
      </div>
    </>
  );
  ...
  return (
    <>
      <div className="wizard-step test-step">
        ...
        <p>
          Send a tiny request through the route you just configured. If capture was off, Exalto
          Capture turns it on only for this disposable test, then restores your previous setting.
        </p>
        {!captureTransportReady && (
          <div className="wizard-warning credential-service-warning" role="status">
            <Network size={13} />
            <span>
              The trusted capture transport is not ready. No Exalto Seal account is required. Wait
              for the trusted connection, then run this disposable test.
            </span>
          </div>
        )}
        <div className="test-prompt-receipt">
          <span>
            <CircleDot size={10} /> REC / SMALL TEST
          </span>
          <strong>{testPrompt}</strong>
          <small>
            Use a low-cost model available to your account. {credentialCopy} Once captured, setup
            can take this exact disposable Trace through sealing and local verification.
          </small>
        </div>
        {captureTransportReady && client !== 'builtin' ? (
          <AgentSetup
            ...
            manual={
              <pre>
                <code>{testCommand(client, testPrompt)}</code>
              </pre>
            }
          />
        ) : (
          <div className="connection-instructions test-command is-waiting">
            <div className="instruction-heading">
              <span>WAIT FOR TRUSTED TRANSPORT</span>
              <strong>The disposable command will appear when capture is ready</strong>
            </div>
            <p>
              Capture authenticates the provider exchange through a trusted live transport. This
              check is separate from an Exalto Seal account.
            </p>
          </div>
        )}
        <div className={`test-result is-${status}`} role="status" aria-live="polite">
          <span>
            {status === 'captured' || status === 'unconfirmed' ? (
              <Check size={14} />
            ) : (
              <StatusDot running={state.capture_enabled} warning={!state.capture_enabled} />
            )}
          </span>
          <div>
            <strong>
              {status === 'captured'
                ? 'Test trace captured'
                : status === 'unconfirmed'
                  ? 'Request succeeded, trace not auto-confirmed'
                  : status === 'checking'
                    ? 'Checking local traces'
                    : status === 'not-found'
                      ? 'No new trace yet'
                      : state.capture_enabled
                        ? 'Disposable capture is on'
                        : state.running
                          ? 'Disposable capture is off'
              ...
              {status === 'captured'
                ...
                : status === 'unconfirmed'
                  ...
                  : status === 'not-found'
                    ...
                    : 'Run the test above, then check for its matching response.'}
            </small>
          </div>
        </div>
      </div>
      <div className="wizard-actions split-actions">
        {status === 'captured' || status === 'unconfirmed' ? (
          ...
          <button
            ...
            type="button"
            onClick={onCheck}
            disabled={!captureTransportReady || status === 'checking' || busy}
          >
            {status === 'checking' ? 'Checking…' : 'Check for new trace'}
          </button>
        )}
        {status !== 'captured' && status !== 'unconfirmed' && (
          <button
            className="mac-button is-large"
            type="button"
            onClick={onSkip}
  ...
  return (
    <>
      <div className="wizard-step account-step ready-step">
        ...
        <p>
          Local capture does not require an Exalto account. Connect one now for hosted credits,
          usage, and account-owned sharing, or continue without it.
        </p>
        <dl className="ready-summary">
          <div>
            ...
            <dt>Local service</dt>
            <dd>
              {state.running ? `Running, capture ${state.capture_enabled ? 'on' : 'off'}` : 'Off'}
            </dd>
          </div>
          <div>
            <span>
          ...
          <div>
            ...
            <dt>Sealing service</dt>
            <dd>
              {notaryLabel} · {sealingStatus}
            </dd>
          </div>
          <div>
            <span>
            ...
            <dt>Local vault</dt>
            <dd>{vaultProtection(state.vault_mode).label}</dd>
          </div>
        </dl>
        {disposableTraceId && (
          <div className={`first-proof-ready ${sealingReady ? '' : 'is-blocked'}`}>
            <span>
              <BadgeCheck size={15} />
            </span>
            {sealingReady ? (
              <div>
                <strong>Your first local Trace is ready to seal</strong>
                <small>
                  Exalto Capture will open this exact test Trace, seal it with {notaryLabel}, and
                  verify the portable proof locally. It will stay private unless you explicitly
                  share it.
                </small>
              </div>
            ) : (
              <div>
                <strong>Your first local Trace will stay private for now</strong>
                <small>
                  {notaryLabel} is{' '}
                  {sealingPhase === 'unreachable'
                    ? 'not reachable'
                    : sealingPhase === 'trust_unavailable'
                      ? 'missing trusted endpoint information'
                      : 'still starting'}
                  . Finish setup, then retry the Seal connection before creating a portable proof.
                </small>
              </div>
            )}
          </div>
        ...
        {disposableTraceId && sealingReady ? (
          <>
            <button
              ...
              type="button"
              onClick={() =>
                void onFinish('traces', { traceId: disposableTraceId, action: 'first-proof' })
              }
              disabled={busy}
            >
              {busy ? 'Finishing setup…' : 'Seal and verify test Trace'}
            </button>
            <button
              className="mac-button is-large"
              type="button"
          ...
          <>
            <button
              ...
              disabled={busy}
            >
              {busy ? 'Finishing setup…' : 'Open Capture and retry Seal'}
            </button>
            <button
              className="mac-button is-large"
              type="button"
              onClick={() => void onFinish('traces', { traceId: disposableTraceId })}
              disabled={busy}
            >
              Open test Trace
            </button>
          ...
          <>
            <button
              ...
              disabled={busy}
            >
              {busy ? 'Finishing setup…' : 'Open Capture'}
            </button>
            <button
              className="mac-button is-large"
              type="button"
              onClick={() => void onFinish('traces')}
              disabled={busy}
            >
              Open Traces
            </button>
