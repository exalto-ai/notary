...
export function HomeView({
  ...
  const sealingNeedsAttention =
    sealingPhase === 'unreachable' || sealingPhase === 'trust_unavailable';
  ...
  return (
    <div className="native-page capture-page">
          ...
          <div>
            ...
            <p>
              {recording
                ...
                : 'Start capturing before your next request. Requests sent while capture is off cannot be sealed later.'}
            </p>
          </div>
        </div>
        {recording ? (
      ...
      {captureToast && (
        <div key={captureToast} className="capture-toast" role="status" aria-live="polite">
      ...
      {sealingStarting && (
        <div className="capture-warning is-starting" role="status">
          <div className="capture-warning-copy">
            <strong>{sealingServiceName} is starting</strong>
            <span>
              Checking the trusted capture transport. Capture will be available when this check
              succeeds. An Exalto Seal account is not required.
            </span>
          </div>
        </div>
      )}

      {sealingNeedsAttention && (
        <div className="capture-warning" role="status">
          <div className="capture-warning-copy">
            <strong>
              {sealingPhase === 'unreachable'
                ? `${sealingServiceName} cannot be reached`
                : 'Sealing trust needs attention'}
            </strong>
            <span>
              {state.sealing_service_readiness.message ??
                (sealingPhase === 'unreachable'
                  ? 'The trusted endpoint did not complete its transport handshake.'
                  : 'The configured trust source did not resolve a trusted sealing endpoint.')}
            </span>
            <span>
              Capture needs this trusted transport, but it does not require an Exalto Seal account.
              Restore the connection, then start capturing.
            </span>
          </div>
          <div className="capture-warning-actions">
            <button type="button" onClick={onRetryConnections}>
              Try again
            </button>
            <button type="button" onClick={() => onNavigate('settings')}>
              Review sealing status
            </button>
          </div>
        </div>
      )}
      ...
      <section className="capture-layout">
        <div className="capture-main-stack">
          ...
          <section className="capture-route-card">
            <header>
              <span className="section-label">What crosses each boundary</span>
              <strong>
                {sealingReady
                  ? `${sealingServiceName} is ready to receive ciphertext.`
                  : `${sealingServiceName} is not used until it is ready.`}{' '}
                Plaintext stays between this Mac and the provider.
              </strong>
            </header>
            <div className="native-route">
              <RouteStop
                title="AI client"
                detail="Credential and plaintext"
                active={state.running}
                tone="local"
              />
              <RouteStop
                title="This Mac"
                detail="Private capture"
                active={recording}
                tone="local"
              />
              <RouteStop
                title={sealingServiceName}
                detail={recording && sealingReady ? 'Ciphertext only' : 'Not used'}
                active={recording && sealingReady}
                tone="seal"
              />
              <RouteStop
                title="Provider"
          ...
          <section className="capture-protection-card">
            ...
            <dl>
              <div>
                ...
                <dd>{`${sealingServiceName} · ${sealingStatus}`}</dd>
              </div>
              <div>
                <dt>Local route</dt>
                <dd>
...
export function RouteStop({
  ...
  return (
    <div className={`native-route-stop is-${tone}`}>
