...
pub(super) async fn restore_temporary_capture(
    ...
    if !was_enabled {
        ...
        let managed_generation = authenticated_managed_generation(process)
            .await
            .ok_or_else(|| {
...
pub(super) async fn begin_temporary_capture(
    ...
    if state.recovery_owner()?.is_some() {
        ...
        {
            return Err(
                "The bundled local service did not retain its authenticated listener during recovery."
                    .into(),
            );
        }
        run_while_window_generation_is_current(
    ...
    let managed_generation = match run_while_window_generation_is_current(
        &mut generation_events,
        window_generation,
        DISPOSABLE_TEST_SETUP_CANCELLED,
    ...
    .await
    {
        ...
        Ok(None) => {
            state.release_owner(&lease_id)?;
            return Err(
                "The bundled local service did not authenticate its listener for the disposable test."
                    .into(),
            );
        }
        Err(error) => {
    ...
    let managed_child = match run_while_window_generation_is_current(
        &mut generation_events,
        window_generation,
        DISPOSABLE_TEST_SETUP_CANCELLED,
    ...
    if let Err(error) = validate_temporary_capture_target(was_enabled, managed_child) {
        state.release_owner(&lease_id)?;
        return Err(error);
    }
    *previous = Some((lease_id.clone(), was_enabled));
    ...
    {
        return Err(
            "The bundled local service lost its authenticated listener during disposable setup. Capture recovery will keep the previous setting safe."
                .into(),
        );
    }
    if !state.owns_live_lease(&lease_id)? {
...
fn validate_temporary_capture_target(
    ...
    if !managed_child {
        return Err(
            "The disposable test requires the exact local service supervised by Exalto Capture. Stop any other service using the capture ports and try again."
                .into(),
        );
    }
    Ok(())
...
pub(super) async fn get_recent_trace_probes(
    ...
    .await?;
    let generation = run_while_window_generation_is_current(
        &mut generation_events,
        expected_generation,
        DISPOSABLE_TEST_CANCELLED,
        authenticated_managed_generation(&process),
    ...
    .await?
    .ok_or_else(|| {
        "The supervised local service is not ready for a disposable test.".to_string()
    })?;
    ...
    if !run_while_window_generation_is_current(
        ...
        expected_generation,
        DISPOSABLE_TEST_CANCELLED,
        same_managed_daemon_is_healthy(&process, generation),
    )
    .await?
    {
        return Err("The supervised local service changed while reading local traces.".into());
    }
    if !temporary_capture.owns_live_lease(&lease_id)? {
        return Err(DISPOSABLE_TEST_CANCELLED.into());
    }
...
pub(super) async fn confirm_disposable_trace(
    ...
    .await?
    .ok_or_else(|| "The supervised local service is not ready for confirmation.".to_string())?;
    let trace_client = client()?;
    ...
    .await?
    {
        return Err("The supervised local service changed while confirming the test trace.".into());
    }
    if !temporary_capture.owns_live_lease(&lease_id)? {
        return Err(DISPOSABLE_TEST_CANCELLED.into());
    }
...
mod tests {
    ...
    use super::{
        SealingServiceIdentity, SealingServiceKind, TemporaryCaptureState, resolve_sealing_service,
        valid_live_lease_id, validate_temporary_capture_target, write_capture_setting_with,
    };
    ...
    fn temporary_capture_never_enables_an_external_service() {
        assert!(validate_temporary_capture_target(false, true).is_ok());
        assert!(validate_temporary_capture_target(true, true).is_ok());
        assert!(validate_temporary_capture_target(true, false).is_err());
        assert!(validate_temporary_capture_target(false, false).is_err());
    }
